...
this.get_japanese = function(str1, str2)
    if this.containsKana(str1) then
        return str1
    elseif this.containsKana(str2) then
        return str2
    elseif this.containsNonSimplifiedChinese(str1) then
        return str1
    elseif this.containsNonSimplifiedChinese(str2) then
        return str2
    else
        return str1
    end
...
this.get_last_two_parts = function(input)
    local lines = {}
...
this.get_japanese_from_subtext = function(text)
    ...
    return this.get_japanese(this.get_last_two_parts(text))
end

this.isKana = function(char)
...
-- 检查字符串是否包含假名
this.containsKana = function(str)
    for i = 1, #str do
        local char = str:sub(i, i + 2)
        if this.isKana(char) then
            return true
        end
    end
    return false
